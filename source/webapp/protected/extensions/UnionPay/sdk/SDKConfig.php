...
//        $configFilePath = dirname(__FILE__) .'/'. "../iniConfig/acp_sdk_pro.ini";
//       // echo $configFilePath;exit;
//        if (!file_exists($configFilePath)) {
//            $logger = LogUtil::getLogger();
//            $logger->LogError("配置文件加载失败，文件路径：[" . $configFilePath . "].请检查启动php的用户是否有读权限。");
//            return;
//        }
        $ini_array = \Yii::app()->params['UnionPay'];//parse_ini_file($configFilePath, true);
        ...
        $this->payFrontUrl = array_key_exists("acpsdk.payFrontUrl", $sdk_array) ? $sdk_array["acpsdk.payFrontUrl"] : null;
        $this->payBackUrl = array_key_exists("acpsdk.payBackUrl", $sdk_array) ? $sdk_array["acpsdk.payBackUrl"] : null;
